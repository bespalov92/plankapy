...
class Planka:
    ...
    def __repr__(self):
        return f"<{type(self).__name__}:\n\t" \
               f"Base URL: {self.url}\n\t" \
               f"Login User: {self.username}\n\t" \
               f"Login Pass: {self.password}\n\t" \
               f"API Token: {self.auth}\n>"
    ...
    def deauthenticate(self) -> bool:
        ...
        try:
            ...
            raise InvalidToken(
                "No active access token assigned "
                f"to this instance\n{self.__repr__()}"
            )
    ...
    def authenticate(self) -> bool:
        ...
        try:
            request = requests.post(
                url=f"{self.url}/api/access-tokens",
                data=data)
    ...
    def request(
        ...
        if response.status_code not in [200, 201]:
            ...
            raise InvalidToken(
                f"Failed to {method} {url} with status code "
                f"{response.status_code}, error message:\n{full_message}"
            )
...
class Controller():
    ...
    def __str__(self) -> str:
        ...
        """
        return f"{type(self).__name__}:\n" \
               f"{json.dumps(self.data, sort_keys=True, indent=4)}"
    ...
    def create(self, route: str, data: dict | None = None) -> dict:
        ...
        if not data:
            raise InvalidToken(
                f"Please Build a {type(self).__name__} before creating"
            )
    ...
    def update(self, route: str, data: dict | None = None) -> dict:
        ...
        if not self.data:
            raise InvalidToken(
                f"Please Build a {type(self).__name__} before updating"
            )
    ...
    def last_response(self) -> dict:
        ...
        if self.response:
            ...
            raise InvalidToken("No response")
...
class Project(Controller):
    ...
    def get(self, name: str | None = None, oid: str | None = None) -> dict:
...
class List(Controller):
    ...
    def delete(
        ...
        if not (project_name and board_name and list_name):
            raise InvalidToken(
                "Please provide a project, board, and list names"
            )
        lst = self.get(project_name, board_name, list_name)
...
class Card(Controller):
    ...
    def get(
        ...
        board = board_con.get(project_name, board_name)
        lst_id = [
            ls for ls in board["included"]["lists"]
            if ls["name"] == list_name
        ][0]["id"]
        cards = [
            card for card in board["included"]["cards"]
            if card["listId"] == lst_id
        ]
    ...
    def delete(
        ...
        if not (project_name and board_name and list_name and card_name):
            raise InvalidToken(
                "Please provide a project, board, list, and card name"
            )
...
class Label(Controller):
    ...
    def delete(
        ...
        if not (project_name and board_name and label_name):
            raise InvalidToken(
                "Please provide project, board, and label names"
            )
    ...
    def add(
        ...
        if label_id and card_id:
            return super().create(
                route=f"/api/cards/{card_id}/labels",
        ...
        return super().create(
            route=f"/api/cards/{card['item']['id']}/labels",
            data={"labelId": label['item']['id']}
        )
    ...
    def remove(
        ...
        if card_id:
            label_id = [
                label['id'] for label
                in Card(self.instance).get_labels(oid=card_id)
...
class Task(Controller):
    ...
    def get(
        ...
        if not (project_name and board_name and list_name and card_name):
            raise InvalidToken(
                "Please provide project, board, list, and card names"
            )
        board_con = Board(self.instance)
        board = board_con.get(project_name, board_name)
        list_id = [
        ...
        tasks = [
            task for task in board["included"]["tasks"]
            if task["cardId"] == card_id
        ]
...
class Background(Controller):
    ...
    def apply(self, prj_name: str):
        ...
        if "type" not in self.data.keys():
            raise InvalidToken(
                "Please specify a background type: `gradient` | `image`"
            )
        if (
            self.data["type"] == "gradient"
            and self.data["name"] not in self.options
        ):
            raise InvalidToken(
                f"Gradient {self.data['name']} not found: "
                f"please choose from\n{self.options}"
            )

        return super().update(
            route=f"/api/projects/{prj_id}",
            data={"background": self.data}
        )
...
class User(Controller):
    ...
    def get(self, username: str | None = None):
        """Gets a user
